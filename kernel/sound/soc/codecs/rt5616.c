...
static struct snd_soc_codec *rt5616_codec_codec=NULL;
...
#else
	{RT5616_DAC1_DIG_VOL    , 0xafaf},//yemk add
#endif
	{RT5616_OUT_L3_MIXER	, 0x0278}, //DACL1 -> OUTMIXL
	{RT5616_OUT_R3_MIXER	, 0x0278}, //DACR1 -> OUTMIXR
	/*LOUT*/
	{RT5616_LOUT_MIXER	, 0xc000},
#if defined(CONFIG_MALATA_D7005) || defined(CONFIG_MALATA_D8006) || defined(CONFIG_MALATA_D8009)||defined(CONFIG_MALATA_D7022)
	{RT5616_LOUT_CTRL2	, 0x8000},//enable differential line output
#endif
	/*Capture*/
	{RT5616_REC_L2_MIXER	, 0x0469}, //MIC1 -> RECMIXL
	{RT5616_REC_R2_MIXER	, 0x0469}, //MIC1 -> RECMIXR
...
#if defined(CONFIG_MALATA_D7005)||defined(CONFIG_MALATA_D8009)||defined(CONFIG_MALATA_D7022)
	[RT5616_LOUT_CTRL2] = 0x8000,  //enable differential line output
#endif
	[RT5616_INL1_INR1_VOL] = 0x0808,
	[RT5616_DAC1_DIG_VOL] = 0xafaf,
#if defined(CONFIG_MALATA_D8006)//Modfied by SolinLin for the volume of recording is very low
   [RT5616_ADC_DIG_VOL] = 0x3f3f,
#else
...
static void jd_timer_callback(unsigned long data)
{
	int ret = 0;
	struct snd_soc_codec *codec = (struct snd_soc_codec *)data;
	struct rt5616_priv *rt5616 = snd_soc_codec_get_drvdata(codec);
	//printk("<YLM>, in jd_timer_callback.\n");
	ret = mod_timer(&rt5616->jd_timer, jiffies + msecs_to_jiffies(100));
	if (ret) printk("<YLM>, error in mod_timer.\n");
	schedule_work(&rt5616->jd_work);
}
...
{
	struct rt5616_priv *rt5616_codec = NULL;
	if(rt5616_codec_codec == NULL)
		return;
	rt5616_codec = snd_soc_codec_get_drvdata(rt5616_codec_codec);
	if(rt5616_codec == NULL)
		return;

	if(rt5616_codec->spk_ctrl_io)
	{
		//printk("--------%s----------status = %d\n",__FUNCTION__,status);
		if(status){
			...
			gpio_set_value(rt5616_codec->spk_ctrl_io, GPIO_HIGH);
#else
			gpio_set_value(rt5616_codec->spk_ctrl_io, status);
#endif
		}else{
			gpio_set_value(rt5616_codec->spk_ctrl_io, status);
		}
...
static void hp_ctrl_fun(bool on)
{
	struct rt5616_priv *rt5616;
	//printk("--------%s----------status = %d\n",__FUNCTION__,status);
	if(!rt5616_codec_codec)
		return;

	rt5616 = snd_soc_codec_get_drvdata(rt5616_codec_codec);
	if(!rt5616)
		return;

	if (on) {
		snd_soc_update_bits(rt5616->codec, RT5616_HP_VOL,
...
static void hp_select_int_mic(bool internal_mic)
{
	struct rt5616_priv *rt5616;
	if(!rt5616_codec_codec)
		return;
	...
	if (internal_mic) {
		snd_soc_update_bits(rt5616->codec, RT5616_REC_L2_MIXER, 0x0006, 0x0002);
		snd_soc_update_bits(rt5616->codec, RT5616_REC_R2_MIXER, 0x0006, 0x0002);
	} else {
		snd_soc_update_bits(rt5616->codec, RT5616_REC_L2_MIXER, 0x0006, 0x0004);
		snd_soc_update_bits(rt5616->codec, RT5616_REC_R2_MIXER, 0x0006, 0x0004);
	}
}

static void hp_mic_detection(void)
{
	int i;
	int adc_val = 0;
	//printk("yemk hp_record_status %d\n",hp_record_status);
	if(!hp_record_status)
	{
		for(i=0;i<10;i++)
			adc_val += adc_sync_read(hprecordclient);
		//printk("<YLM>, in hp_mic_detection, adc_val = %d.\n", adc_val);
		if ((adc_val <= ADC_VAL_MIC_HIGH) && (adc_val > ADC_VAL_MIC_LOW)){
			hp_select_int_mic(false);
			hp_record_status=1;
		}
	}
	/*else {
		hp_select_int_mic(true);
		hp_record_status=0;
	}*/
}

static void jd_timer_work(struct work_struct *work)
{
	int ret;
	int hp_jd_sts;

	//printk("<YLM>, in jd_timer_work.\n");
	struct rt5616_priv *rt5616 = container_of(work, struct rt5616_priv, jd_work);

	ret = snd_soc_read(rt5616->codec, RT5616_INT_IRQ_ST);
	if (ret < 0) {
		dev_err(rt5616->codec->dev, "Failed to read reg: %d\n", ret);
		return;
	} else {
		hp_jd_sts = (ret & RT5616_STA_GP_JD) >> RT5616_STA_GP_JD_BIT;
		//printk("<YLM>, in jd_timer_work, jack data is 0x%x, hp_jd_sts is %d, rt5616->last_hp_jd_sts is %d.\n", ret, hp_jd_sts, rt5616->last_hp_jd_sts);
		if (rt5616->last_hp_jd_sts != hp_jd_sts) {
			rt5616->last_hp_jd_sts = hp_jd_sts;
			/*
			if(hp_jd_sts == HP_JD_INSERT){
				snd_soc_dapm_force_enable_pin(&rt5616->codec->dapm, "micbias1");
				snd_soc_dapm_sync(&rt5616->codec->dapm);
			}else{
				snd_soc_dapm_disable_pin(&rt5616->codec->dapm, "micbias1");
				snd_soc_dapm_sync(&rt5616->codec->dapm);

			}
			*/
			switch_set_state(&rt5616->headset_sdev, (hp_jd_sts == HP_JD_INSERT ? 1 : 0));
			if (hp_jd_sts != HP_JD_INSERT) {
				hp_select_int_mic(true);
				hp_record_status=0;
			} else if (!rt5616->hdmi_ndet) {
				/* enable HP playback when HDMI pluged, xmylm, 20130718
				because that rt5616_mute will not be called when HDMI pluged, So enable HP output in detection loop*/
				hp_ctrl_fun(true);
			}
		}
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		#if defined(CONFIG_MALATA_D1004)
		snd_soc_update_bits(codec, RT5616_EQ_CTRL1, RT5616_EQ_SRC_MASK, RT5616_EQ_SRC_ADC); //bard 7-11
		rt5616_update_eqmode(codec, LOUT); //bard 7-11
		#endif
...
static int rt5616_probe(struct snd_soc_codec *codec)
{
	...
	rt5616->last_hp_jd_sts = -1;
	rt5616->headset_sdev.name = "h2w";
	ret = switch_dev_register(&rt5616->headset_sdev);
	if (ret) {
		printk("<YLM>, Unable to register switch device.\n");
		return ret;
	}

	INIT_WORK(&rt5616->jd_work, jd_timer_work);
	setup_timer(&rt5616->jd_timer, jd_timer_callback, (unsigned long)codec);
	//printk("<YLM>, starting timer in %ld, HZ is %d.\n", jiffies, HZ);
	//ret = mod_timer(&jd_timer, jiffies + msecs_to_jiffies(5000));
	//if (ret) printk("<YLM>, in rt5616_probe, Error in mod_timer\n");
	rt5616->jd_timer.expires  = jiffies + 1*HZ;
	add_timer(&rt5616->jd_timer);

	if(rt5616->spk_ctrl_io)
	{
		ret = gpio_request(rt5616->spk_ctrl_io, "rt5616_spk_ctrl");
	    if (ret){
	        printk("r5616_control request gpio fail!\n");
			return ret;
	    }
	    gpio_direction_output(rt5616->spk_ctrl_io, GPIO_LOW);
	    gpio_set_value(rt5616->spk_ctrl_io, GPIO_LOW);
	}
	rt5616_reset(codec);
	snd_soc_update_bits(codec, RT5616_PWR_ANLG1,
		RT5616_PWR_VREF1 | RT5616_PWR_MB |
	...
	hprecordclient = adc_register(2, NULL, NULL);
...
static int rt5616_remove(struct snd_soc_codec *codec)
{
	...
	ret = del_timer(&rt5616->jd_timer);
	if (ret) printk("==rt5616==, jd_timer delete failed!\n");
...
static int rt5616_suspend(struct snd_soc_codec *codec, pm_message_t state)
{
	...
	if (rt5616->pdata->fb_status && rt5616->pdata->fb_lcd_drv_ctl) {
		if (!rt5616->pdata->fb_status(FB_STATUS_GET)) {
			printk("%s, shut down lcd bl power.\n", __FUNCTION__);
			rt5616->pdata->fb_lcd_drv_ctl(0);
		}
	}
	rt5616_set_bias_level(codec, SND_SOC_BIAS_OFF);
	return 0;
...
{
	struct rt5616_priv *rt5616_codec;
	if(!rt5616_codec_codec)
		return;
...
static int rt5616_mute(struct snd_soc_dai *dai, int mute)
{
	struct snd_soc_codec *codec = dai->codec;
	struct rt5616_priv *rt5616 =snd_soc_codec_get_drvdata(codec);
	...
	if (mute) {
		...
		if (rt5616->last_hp_jd_sts == HP_JD_INSERT) {
			hp_ctrl_fun(true);
		} else {
			if(rt5616->hdmi_ndet) {
				spk_ctrl_fun(GPIO_HIGH);
#if defined(CONFIG_MALATA_D7005)||defined(CONFIG_MALATA_D7806)||defined(CONFIG_MALATA_D1012)||defined(CONFIG_MALATA_D8009)||defined(CONFIG_MALATA_D7022)||defined(CONFIG_MALATA_D1013)
				msleep(40);    // To waiting PA's dalay, xmylm, bug-id<9546>, 20130716.
#endif
			}
		}
	}
		    ...
		    const struct i2c_device_id *id)
{
	struct rt5616_priv *rt5616;
	int ret;
	struct rt5616_codec_platform_data *pdata = i2c->dev.platform_data;
	printk("enter %s\n",__func__);
	...
	rt5616->pdata = pdata;
	rt5616->spk_ctrl_io = pdata->spk_ctl_io;
	if(pdata->io_init){
		ret =  pdata->io_init();
		if (ret < 0) {
